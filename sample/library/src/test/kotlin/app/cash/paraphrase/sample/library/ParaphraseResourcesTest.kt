...
class ParaphraseResourcesTest {
  ...
  private val paraphraseResources = ParaphraseResources(JvmDateTimeConverter)

  @Test fun date() {
    val formattedResource =
      paraphraseResources.library_date_argument(LocalDate.of(2023, Month.NOVEMBER, 3))
  ...
  @Test fun time() {
    val formattedResource =
      paraphraseResources.library_time_argument(LocalTime.of(14, 37, 21))
    val result = MessageFormat(stringResolver.getString(formattedResource.id), Locale.US)
      .format(formattedResource.arguments)
